...
namespace AASTHA2.Validator
{
    public class PatientValidator : AbstractValidator<PatientDTO>
    {
        public PatientValidator(ServicesWrapper ServicesWrapper)
        {
            ...
            RuleFor(m => new { m.id, m.firstname, m.middlename, m.fathername, m.lastname }).SetValidator(new ExistPatientValidator(ServicesWrapper)).WithMessage("Patient already exist.");
        }
    }
}